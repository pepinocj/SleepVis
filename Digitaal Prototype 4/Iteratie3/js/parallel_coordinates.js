...
function sleep_time_context(container){
    var dataPoints = collection_day_data.map(function(d,i){
        ...
        return {col:i, value: d.smartPhoneData.flattened["TST"] };
    });
    ...
    var dataPoints2 = collection_day_data.map(function(d,i){
        ...
        return {col:i, value: d.smartPhoneData.flattened["TWT"]+d.smartPhoneData.flattened["TST"] };
    });
    ...
    var bars = svg.append("g").attr("id","bars_history_"+name)
        ...
        .data(points)
        .enter()
        .append('g').attr("class",function(d,i){return "bar day"+i+" "+name;})
...
function small_bar_chart_context_default(container){
    ...
    var dataPoints = visualizedDays.map(function(d,i){
        ...
        console.log(d.nrDay);
        return {col:i, value:temp, day:d.nrDay};
    });
...
//    xTimeYearScale.range([ 0, width ]).domain([new Date(collection_day_data[0].smartPhoneData.shown_date),new Date(collection_day_data[collection_day_data.length-1].smartPhoneData.shown_date)]);
    ...
    dataPoints.map(function(d,i){
        dataPointsArea.push({col:d.col, value1:d.value, value2:dataPoints2[i].value});
    })
    
    var area = d3.svg.area()
    .x(function(d) { return width/(dataPoints.length*4) + x(d.col); })
    .y0(function(d) { return y(d.value2); })
    ...
    .attr("d", area);
        ...
        .data(dataPoints2)
        .enter()
        .append('g').attr("class",function(d,i){return "bar day"+d.day+" "+tbt;})
...
//        .attr('width',function(d){ return width/(dataPoints.length*2) });
    ...
    bars2.append("text")
        .attr("class", "subjectiveFeatureValue" )
        .text(function(d){return d.value})
        .attr({'x':function(d){ return x(d.col); },'y':function(d,i){ return y(d.value)-10 }})

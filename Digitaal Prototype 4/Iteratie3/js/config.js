...
var config_features = {N:{color:colors_blue_to_red, min:30 , max: 120,type:'heart',active:true ,name:"bpm",extra_info:"Beats per minute"},
    meanNN:{color:colors_blue_to_red, min:500 , max: 900,type:'heart',active:false,name:"meanNN",extra_info:"Mean of beat to beat intervals"}
    ,SDNN:{color:colors_blue_to_red, min:0 , max: 500,type:'heart',active:false,name:"SDNN",extra_info:"Standard deviation of beat to beat intervals"}
    ,RMSSD:{color:colors_blue_to_red,min:50 , max: 550,type:'heart',active:false,name:"RMSSD",extra_info:"Root mean square of SDNN"} //wrong
    ...
    ,FFT_LFnu:{color:colors_green_to_red,min:0, max:1,type:'heart',active:true,name:"LFnu",extra_info:"Indication of sympathetic activity of ANS (stress) and REM sleep"}
    ,FFT_HFnu:{color:colors_green_to_red,min:0, max:1,type:'heart',active:true,name:"HFnu",extra_info:"Indication of parasympathetic activity of ANS (rest)"}
//    //,FFT_VLF:{color:colors_green_to_red, min:0, max:50000,type:'heart'}
    ...
    //,FFT_TotalPower:{color:colors_green_to_red, min:0, max:100000,type:'heart'}
    ,FFT_LFHFratio:{color:colors_green_to_red,min:0, max:3,active:false,type:'heart',name:"LFnu/HFnu",extra_info:"Indication of sympathovagal balance"}
    ...
    ,movement:{color:colors_white_to_red,min:0, max:50,type:'mov',active:true,name:"movement",extra_info:"movement intensity"}
    //,movement_std:{color:colors_blue_to_red,min:0, max:10,type:'mov',active:false,name:"movement_std"}
    ,mean_x_norm:{color:colors_green_to_red,min:-1, max:1,type:'mov',active:false,name:"x"}
    ,mean_y_norm:{color:colors_green_to_red.slice().reverse(),min:-1, max:1,type:'mov',active:false,name:"rate standing-lying"}
    ,mean_z_norm:{color:colors_green_to_red,min:-1, max:1,type:'mov',active:false,name:"z"}

    ,mean_position:{color:colors_body_positions,min:-3.14,max:3.14,type:'mov',active:false,name:"lying position"}
};
...
function addPostProcessingFeatures(){
    ...
    config_features["standing"] = {color:colors_body_positions,min:0,max:1,type:'mov',active:true,name:"standing periods"};
    config_features["lying"] = {color:colors_body_positions,min:0,max:1,type:'mov',active:true,name:"lying periods"};
   // add3DPosition();
}

function add3DPosition(){
    config_features["mean_3D_position"] = {color:colors_body_positions,min:0,max:1,type:'mov',active:false,name:"full detail position"};
...
function initializeConfigVariables(){
    ...
    margin = {top: 0, right: 0, bottom: 10, left: 90},
        width = document.body.clientWidth*0.6 - margin.left - margin.right,
        height = 200 - margin.top - margin.bottom;

    detailsMargin = {top: 0, right: 10, bottom: 10, left: 0};
   ...
   brush = d3.svg.brush()
    ...
    config_subjective_durations = {simple:{ min:0 , max: 5},
    durable:{min:0 , max: 240}
    ,sleep:{min:0 , max: 800}};
...
sleep_activities.map(function(d){
    ...
    bed_times = ["TST","WASO","EMA","SOL"];
    event_activities = ["M","C","A","Dutjes","nightmare","light_off","light_on","Fat_food","sport","toilet","tv"];
...
function contextMappingGeneration(){
    ...
    contextMapping = {
        ...
        A:imageFolder+"glass-and-bottle-of-wine.png",
        M:imageFolder+"pill.png",
        Fat_food:imageFolder+"fast-food.png",
        C:imageFolder+"koffie.png",
        dining:imageFolder+"icon-dining.png",
        nightmare:imageFolder+"scary_mask-512.png",
        toilet:imageFolder+"toilet.png",
        light_off:imageFolder+"OffLamp-icon.png",
        light_on:imageFolder+"OnLamp-icon.png",
        tv:imageFolder+"tv.png"

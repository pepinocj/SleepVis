...
d3.gantt = function() {
    ...
    function generateSubjectiveSingleDetail(obj, container){
        ...
        container.text(function(d){
            ...
            if (d.endDate == d.startDate){
                ...
                return translator[ d.status]+":duur "+(new Date(d.endDate-d.startDate)).toLocaleTimeString() +" \n"+" van "+ d.startDate.toLocaleTimeString() +"\n tot "+ d.endDate.toLocaleTimeString() ;
            }
    ...
    function gantt(tasks,container) {
        ...
        svg.selectAll(".chart")
            ...
            .attr("class", function(d){
                if(taskStatus[d.status] == null){ return "bar";}
                return "activityDuration  bar "+ taskStatus[d.status]+" day"+day;
            })
            .attr("y",-5 )
            .attr("x",function(d){
        ...
        svg.selectAll(".chart")
            ...
            .attr("class", function(d){
                if(taskStatus[d.status] == null){ return "bar";}
                return "activityNoDuration bar "+ taskStatus[d.status]+" day"+day;
            })
            .attr("y",-5 )
            .attr("transform", triTransform)
           // .attr("d",arc)

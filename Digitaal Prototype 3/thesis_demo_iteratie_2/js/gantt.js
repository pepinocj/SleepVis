...
function generateSubjectiveOverviewAllFeatures(container,listDays){
    ...
    listDays.map(function(nrDay){
        ...
        console.log(dayData)
        var allActivities = {a:dayData["bed_times_after_sleep"],b:dayData["nightActivities"]};
       // var filteredAct = allActivities.filter(function(d){return feature == d.label});
        ganttWrapperFunction(allActivities,container,collection_day_data[nrDay].date,new Date(collection_day_data[nrDay].date.getTime()+24*60*60*1000),nrDay,timeFeatures.brusher.width,10,0);

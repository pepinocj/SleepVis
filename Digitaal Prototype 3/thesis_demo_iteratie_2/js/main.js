...
function getEpochData(day,epoch){
    ...
    for (var i = 0 ; i<dayData.length;i++){
        if(dayData[i].col == epoch){
            ...
            if(dayData[i].feature == "FFT_VLF" || dayData[i].feature == "FFT_LF"|| dayData[i].feature == "FFT_HF"){
                
                 result[dayData[i].feature] = Math.log(dayData[i].value) / Math.LN10;
                 if(dayData[i].value == 0){
                     result[dayData[i].feature] = 0;
                 }
            }
...
function getDataPerEpoch(day){
    ...
    for (var i = 0 ; i<dayData.length;i++){
            ...
            if(dayData[i].feature == "FFT_VLF" || dayData[i].feature == "FFT_LF"|| dayData[i].feature == "FFT_HF"){
                 ...
                 result[dayData[i].col][dayData[i].feature] = Math.log(dayData[i].value) / Math.LN10;
                 if(dayData[i].value == 0){
                     result[dayData[i].col][dayData[i].feature] = 0;
                 }
            }
...
function visualizeDay(nrDay,cellHeightDayView){
    ...
    dayViz.on("mouseover",function(){
        ...
        d3.select("#dayInspecting").text(dayContextData.shown_date);
    });




    dayViz.append("div").text(dayContextData.shown_date).attr("class","dayTitle");
    ...
    var buttonDiary = timeLineSubjContext.append("div").attr("class","showDiary").append("img")// Moet in svg
    ...
    collection_day_data[nrDay]["ganttChart"] = addTimeLineSubjContext(timeLineSubjContext,dayContextData,collection_day_data[nrDay].date,new Date(collection_day_data[nrDay].date.getTime()+24*60*60*1000),nrDay,width,heightGantt,90);

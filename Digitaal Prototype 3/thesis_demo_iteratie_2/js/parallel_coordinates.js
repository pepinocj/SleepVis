...
function small_bar_chart_context(container,feature){
    var dataPoints = visualizedDays.map(function(d,i){
        var temp = 0;
        if(collection_day_data[d].smartPhoneData.flattened[feature] != undefined){
            temp = collection_day_data[d].smartPhoneData.flattened[feature];
        }
        return {col:i, value:temp, day:d};
    });
    ...
    xTimeYearScale.range([ 0, width ]).domain([new Date(collection_day_data[visualizedDays[0]].smartPhoneData.shown_date),new Date(collection_day_data[visualizedDays[visualizedDays.length-1]].smartPhoneData.shown_date)]);
    ...
    var xAxis = d3.svg.axis().tickFormat(d3.time.format("%d %b"))
        ...
        .scale(xTimeYearScale) .orient("bottom")
        ;
    ...
    var bars = svg.append("g").attr("id","bars_history")
        ...
        .data(dataPoints)
        .enter()
        .append('g').attr("class",function(d,i){return "bar day"+d.day+" "+feature;})
        .on("mouseover", function(d,i){
            d3.selectAll(".day"+d.day+"."+ feature).classed("subjectiveFeatureSelected",true);

        })
        .on("mouseout", function(d,i){
            d3.selectAll(".day"+d.day+"."+ feature).classed("subjectiveFeatureSelected",false);

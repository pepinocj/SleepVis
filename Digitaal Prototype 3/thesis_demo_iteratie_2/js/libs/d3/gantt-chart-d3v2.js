...
d3.gantt = function() {
    ...
    var triTransform = function(d) {
        return "translate(" + (x(d.startDate)-y.rangeBand()/2) + "," + (y(d.taskName)+5) + ")";
    };
    ...
    function generateSubjectiveSingleDetail(obj, container){
        ...
        container.text(function(d){
            ...
            if (d.endDate == d.startDate){
                return  translator[ d.status]+": \n"+" om "+ d.startDate.toLocaleTimeString() ;
            }else{
                var difDate =new Date(d.endDate.getTime()-d.startDate.getTime());
                difDate = new Date(difDate.valueOf()+difDate.getTimezoneOffset() * 60000).toLocaleTimeString();
                
                return translator[ d.status]+":duur "+difDate +" \n"+" van "+ d.startDate.toLocaleTimeString() +"\n tot "+ d.endDate.toLocaleTimeString() ;
            }
    ...
    function gantt(tasks,container) {
        ...
        svg.selectAll(".chart")
            ...
            .append("svg:image")
            .attr("xlink:href", function(d){return contextMapping[d.status]})
        ...
        .on("mouseover", function(d){
                ...
                d3.select("#dayViz"+day).select(".overlay").append("rect").attr("id","overlappingGantt").attr("style","fill-opacity:0;  width:"+Math.max(x(d.endDate) - x(d.startDate),2)+"; height:100%;z-index:1000; opacity:0.75;stroke-width:4; x:"+x(d.startDate)).attr("class",taskStatus[d.status]);

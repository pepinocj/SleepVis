...
function addSleepContext(scales,label,content,container,nrDay){
    ...
    var x_xis2 = scoreSVG.append('g')
        .attr("class","axis")
        .attr("transform", "translate("+offset+","+(height+25)+")")
        .attr('id','xaxis2')
...
//            }
    ...
    var stackCol = content.filter(function(d){
        return ["WASO","EMA","SOL"].indexOf(d.key)> -1;
    });
    ...
    var seContent = content.filter(function(d){return d.key != "TWT"});
    ...
    scoreSVG.append('g')
        .attr("class","axis")
        .attr("transform", "translate("+0+","+(height+30)+")").append("text").text("SE (%)");
    ...
    var se = scoreSVG.append('g')
        ...
        .attr({
            ...
            'y':function(d,i){ return height+20; }})
    ...
    se.append('text').attr("class", "subjectiveFeatureValue" )
        .attr({'style':function(d){if(d.key == "TST"){return "opacity:1";}else{return "";}},
            ...
            'y':function(d,i){ return height+25+(height*0.5/content.length); }})
...
function addTimeLineSubjContext(timeLineSubjContext,dayData,startDate,endDate,nrDay,width,height,left_margin){
    ...
    return ganttWrapperFunction({Bed:dayData["subjective_data"]["bed_times_after_sleep"],Activities:dayData["subjective_data"]["night_activities"]},timeLineSubjContext,startDate,endDate,nrDay,width,height,left_margin);
}

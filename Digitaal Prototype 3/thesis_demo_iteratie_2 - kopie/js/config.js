...
var config_features = {N:{color:colors_blue_to_red, min:300 , max: 600,type:'heart',active:true ,name:"bpm"},
    meanNN:{color:colors_blue_to_red, min:500 , max: 900,type:'heart',active:false,name:"meanNN"}
    ,SDNN:{color:colors_blue_to_red, min:60 , max: 250,type:'heart',active:false,name:"SDNN"}
    ,RMSSD:{color:colors_blue_to_red,min:50 , max: 550,type:'heart',active:false,name:"RMSSD"} //wrong
    ...
    ,pNN50:{color:colors_blue_to_red, min:0 , max: 1,type:'heart',active:true,name:"pNN50"}
    ...
    ,FFT_VLF:{color:colors_blue_to_red, min:0, max:100000,type:'heart',active:true,name:"VLF"}
    ,FFT_LF:{color:colors_blue_to_red, min:0, max:1000,type:'heart',active:true,name:"LF"}
    ,FFT_HF:{color:colors_blue_to_red, min:0, max:1000,type:'heart',active:true,name:"HF"}
    ...
    ,movement:{color:colors_white_to_red,min:67, max:78,type:'mov',active:true,name:"movement"}
...
var visualizedDays = [0,0,0,1,2,3,4,5];
...
function initializeConfigVariables(){
    ...
    margin = {top: 0, right: 0, bottom: 10, left: 90},
        width = document.body.clientWidth*0.66 - margin.left - margin.right,
   ...
   brush = d3.svg.brush()
    ...
    var durable_activities = ["WASO","EMA","SOL","sport","TWT","toilet"];
...
sleep_activities.map(function(d){
    ...
    all_activities = simple_activities.concat(durable_activities).concat(sleep_activities);
...
function provideRoomForSevenDays(){
    var newMargin = document.body.clientHeight - 7*d3.select("#chart").select(".dayViz").node().getBoundingClientRect().height;
    d3.select("#chart").selectAll(".dayViz").attr("style","margin-bottom:"+Math.max(newMargin/7,0));
}

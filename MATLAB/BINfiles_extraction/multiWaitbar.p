...
%   Author: Ben Tordoff
%   Copyright 2007-2012 The MathWorks Ltd

if nargin==0
    help multiWaitbar
    return;
elseif nargin==1 && ischar(label) && strcmpi(label,'ver')                       % Return the version number when asked for it
    temp = '$Revision: 590 $';
    temp([1,end]) = [];
    temp = [mfilename,' ', temp];
    cancel = temp;
...
% -------------------------------------------------------------------------
% -- Project   : Stingray
% -- Content   : This function add, remove or update an entry on the 
%                multi-waitbar.
% -- Filename  : $HeadURL: http://imecwww/svn/stingray/trunk/Data/GlobalLib/MatlabLib/multiWaitbar.m $

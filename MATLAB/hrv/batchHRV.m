...
if ~flagNoGui
    ...
    h.lblHeader = uicontrol(h.panel,'Style','text',...
                'String','# of Header Rows:','HorizontalAlignment','right',...
...
function hrv=batchGetHRV(fList,opt,flagNoGui)
    ...
    for f=1:nFiles 
        ...
        avgElapsed=elapsedTime/f; %average time elapsed to process a single file
        timeRem=ceil(avgElapsed*(nFiles-f)/60); %time remaining in processing (min)        
        %Update waitbar
         bar=round(f/nFiles*100);
        if ~flagNoGui           
            waitbar(bar/100,h,['Processing: ' num2str(bar) '% complete. (~' num2str(timeRem) ' min)'])
            drawnow;
            if abort
            ...
            disp(['Processing: ' num2str(bar) '% complete. (~' num2str(timeRem) ' min)'])
        end
    end
...
function output=getHRV(f,settings)
    ...
    output.nl=nonlinearHRV(nIBI,settings.m,settings.r,settings.n1,settings.n2,settings.breakpoint);        
    %Poincare
    output.poincare=poincareHRV(nIBI);

    %Time-Freq          
    output.tf=timeFreqHRV(dIBI,nIBI,settings.VLF,settings.LF,settings.HF,settings.AROrder, ...
        settings.tfWinSize,settings.tfOverlap,settings.Points,settings.Interp,{'ar','lomb','wavelet'});
    %%%%%%%%%%%%%%%%%%%%%%%
